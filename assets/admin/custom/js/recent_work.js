$(document).ready(function(){
    ...
    $('#recent_work').submit(function(){
          $('.validation-error').html('');
          $("#spinner").show();
          $(".cat-btn").hide();
          var baseurl   = $("#base").val();
          var form_data = $("#recent_work").serializeArray();
             $.ajax({
                  type:'POST',
                  dataType:'json',

                
                 url:baseurl+'addrecent_work',
                  ...
                  { 
                      ...
                      if(data.res == 1)
                       { 
                          success(data.msg);
                          setTimeout(function(){ window.location=baseurl+'recent_work'; }, 700);
  ...
  $(document).on('click','.uploadbtnlarge',function(){
         ...
         $(document).on('click','.del-recent_work',function(){
            $('.main-card-title').html('Edit Recent Work');
           var id = $(this).attr('id');
           ...
           alertify.confirm("Are you sure ?.",
             ...
             }).set({title:"Confirm delete Recent Work"}).set({labels:{ok:'Delete', cancel: 'Cancel'}});
        });
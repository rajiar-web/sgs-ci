                ...
                <tbody><?php if(!empty($plist))
                  {
                    ...
                    {
                     $img = '';
                      $images = json_decode($l->s_image);
                      if(!empty($images))
                      {
                        $pathinfo = pathinfo($images[0]);
                        if(file_exists('.assets/front/assets/images/slider/'.$pathinfo['dirname'].'/60_60_'.$pathinfo['basename']))
                        {
                          $pp = base_url('assets/front/assets/images/slider/'.$pathinfo['dirname'].'/60_60_'.$pathinfo['basename']);
                          $img = '<img src="'.$pp.'">';
                        }
                        else
                        {
                          $pp = base_url('assets/front/assets/images/slider/'.$images[0]);

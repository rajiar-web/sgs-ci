$(document).ready(function(){
    ...
    $('#cform').submit(function(){
         ...
         form_data.push({name: 'desc', value: CKEDITOR.instances.desc.getData()});  
             $.ajax({
                ...
                url:baseurl+'top-products-action',
                  data:form_data,
                  ...
                  { 
                      ...
                      if(data.res == 1)
                       { 
                          success(data.msg);
                          setTimeout(function(){ window.location=baseurl+'top-products'; }, 700);
                       }
                       else
                    {
  ...
  $(document).on('change','#inputlargefile',function(){
                 ...
                 $.ajax({
                     url : baseurl+'top-products-image', 
                     type: "POST",
                     dataType:'json',
                     data: form_data,
                     contentType: false,  
  ...
  $(document).on('click','.uploadbtnlarge',function(){
         ...
         $(document).on('click','.del-top-products',function(){
            $('.main-card-title').html('Delete Top Products ');
           var id = $(this).attr('id');
           ...
           alertify.confirm("Are you sure ?.",
             function(){
                   ...
                   $.ajax({
                           url : baseurl+'delete-top-products',
                           type : 'post',
                           data : form_data,
                           cache: false,
                           contentType: false,
             ...
             }).set({title:"Confirm delete Top Product"}).set({labels:{ok:'Delete', cancel: 'Cancel'}});
        });
